...
export const consoleObservabilitySink: ObservabilitySink = { write(value) { const { event, requestId, seriesId, episodeId, resourceId, actorId, status, errorCode, prismaErrorType, prismaCode, prismaMeta, blockingCount, warningCount, durationMs } = value; const entry = { event, requestId, seriesId, episodeId, resourceId, actorId, status, errorCode, prismaErrorType, prismaCode, prismaMeta, blockingCount, warningCount, durationMs, timestamp: new Date().toISOString() }; if (errorCode || event.endsWith('failure')) console.error(JSON.stringify(entry)); else console.info(JSON.stringify(entry)); } };
