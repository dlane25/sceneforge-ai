...
function sanitizedPrismaDiagnostics(error: unknown): { prismaErrorType?: 'validation'; prismaCode?: string; prismaMeta?: Record<string, string | string[]> } {
  if (error instanceof Prisma.PrismaClientValidationError) return { prismaErrorType: 'validation' };
...
export async function apiError(error: unknown, fallbackCode = 'REQUEST_FAILED', fallbackMessage = 'Request could not be completed', request?: Request): Promise<NextResponse> {
  ...
  if (error instanceof Prisma.PrismaClientValidationError) return response(400, { error: { code: fallbackCode, message: fallbackMessage, requestId } }, requestId);
  if (error instanceof InvalidStateError) return response(error.status, { error: { code: error.code, message: 'The requested operation is not valid in the current state', requestId } }, requestId);
