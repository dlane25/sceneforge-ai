...
export interface ExportConfig { engine: ExportEngineId; ffmpegPath: string; mediaRoot?: string; outputRoot?: string; mediaStorageUri?: string }
...
export function loadExportConfig(env: Environment = process.env): ExportConfig {
  ...
  const config: ExportConfig = { engine: selected, ffmpegPath: env.FFMPEG_PATH || 'ffmpeg', mediaRoot: env.EXPORT_MEDIA_ROOT, outputRoot: env.EXPORT_OUTPUT_ROOT, mediaStorageUri: env.MEDIA_STORAGE_URI || env.VERTEX_OUTPUT_STORAGE_URI || env.GOOGLE_CLOUD_VIDEO_OUTPUT_URI };
