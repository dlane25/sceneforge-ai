...
export async function POST(request: Request, { params }: { params: Promise<{ id: string; episodeId: string }> }) { try { const { user } = await requireUser(); const { id, episodeId } = await params; return NextResponse.json({ data: await productionService.createScene(user, id, episodeId, sceneInputSchema.parse(await request.json())) }, { status: 201 }); } catch (error) { return apiError(error, 'SCENE_CREATE_FAILED', 'Unable to create scene', request); } }
